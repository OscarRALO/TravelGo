...
class FavoritesFragment : Fragment() {
    ...
    private fun removeFavorite(destination: Destination) {
        ...
        val wasRemoved = favoritesPreferences.removeFavorite(destination.id)

        if (wasRemoved) {
            // Actualizar adapter con animación
            adapter.removeItem(destination)
            favoriteDestinations = favoriteDestinations.filterNot { it.id == destination.id }

            updateFavoritesCount()
            showEmptyState(favoriteDestinations.isEmpty())

            Snackbar.make(requireView(), "Eliminado de favoritos", Snackbar.LENGTH_LONG)
                .setAction("Deshacer") {
                    // Restaurar favorito
                    favoritesPreferences.addFavorite(destination.id)
